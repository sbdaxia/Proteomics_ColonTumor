...
k.s.list <-read_tsv("CK_Target/kinsubset_prot_acc_mouse_20170418.gmx.txt")
k.s.list <- k.s.list[-1,]
...
k.s.list.all <-read_tsv("CK_Target/kinsubset_prot_acc_mouse_20170418.gmx.txt")
...
## Set up function
```{r}
phospho_target_info <- function(kinase, all_height=12, sig_height=10) {
  # all target info
  ...
  if (dim(target_phospho)[1] > 1) {
    ...
    figure_name <- paste("CK_Target/", kinase, "_target_phosphorylations_LFC.pdf", sep = "")
    pdf(figure_name,
        width = 8,
        height = all_height)
    par(cex.main=1.1)
    ...
    figure_name <- paste("CK_Target/", kinase, "_target_phosphorylations_quant.pdf", sep = "")
    pdf(figure_name,
        width = 8,
        height = all_height)
    par(cex.main=1.1)
  ...
  if (dim(sig_target_phospho)[1] > 1) {
    ...
    figure_name <- paste("CK_Target/", kinase, "_sig_target_phosphorylations_LFC.pdf", sep = "")
    pdf(figure_name,
        width = 8,
        height = sig_height)
    par(cex.main=1.1)
    ...
    figure_name <- paste("CK_Target/", kinase, "_sig_target_phosphorylations_quant.pdf", sep = "")
    pdf(figure_name,
        width = 8,
        height = sig_height)
    par(cex.main=1.1)
...
## Csnk1a1 (CK1A)
```{r}
phospho_target_info("CK1A")
figures <- list.files("CK_Target", ".*CK1A.*\\.pdf$")
if (length(figures)>0) {
...
## Csnk1d (CK1D)
```{r}
phospho_target_info("CK1D")
figures <- list.files("CK_Target", ".*CK1D.*\\.pdf$")
if (length(figures)>0) {
  paths <- paste0("CK_Target/", figures)
  include_graphics(paths)
}
...
## Csnk1e (CK1E)
```{r}
phospho_target_info("CK1E")
figures <- list.files("CK_Target", ".*CK1E.*\\.pdf$")
if (length(figures)>0) {
  paths <- paste0("CK_Target/", figures)
  include_graphics(paths)
}
...
## Csnk2a1 (CK2A1)
```{r}
phospho_target_info("CK2A1", 16, 12)
figures <- list.files("CK_Target", ".*CK2A1.*\\.pdf$")
if (length(figures)>0) {
  paths <- paste0("CK_Target/", figures)
  include_graphics(paths)
}
...
## Csnk2a2 (CK2A2)
```{r}
phospho_target_info("CK2A2")
figures <- list.files("CK_Target", ".*CK2A2.*\\.pdf$")
if (length(figures)>0) {
  paths <- paste0("CK_Target/", figures)
  include_graphics(paths)
}
...
## Csnk2b (CK2B)
```{r}
phospho_target_info("CK2B")
figures <- list.files("CK_Target", ".*CK2B.*\\.pdf$")
if (length(figures)>0) {
  paths <- paste0("CK_Target/", figures)
  include_graphics(paths)
}
